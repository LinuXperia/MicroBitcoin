...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, TestChain100Setup)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        spends[i].vout[0].nValue = 11 * CENT * COIN_RATIO;
    ...
    BOOST_CHECK(chainActive.Tip()->GetBlockHash() != block.GetWorkHash(1));
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, TestChain100Setup)
{
    ...
    BOOST_CHECK(chainActive.Tip()->GetBlockHash() == block.GetWorkHash(1));
    BOOST_CHECK(pcoinsTip->GetBestBlock() == block.GetWorkHash(1));
    LOCK(cs_main);
